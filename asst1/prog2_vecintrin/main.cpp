...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_vec_float upperb = _cs149_vset_float(9.999999f);

  for(int i = 0; i < N; i += VECTOR_WIDTH) {
    __cs149_mask maskAlign = _cs149_init_ones(min((int)VECTOR_WIDTH, N - i));

    __cs149_vec_float result = _cs149_vset_float(1);
    __cs149_vec_float base;
    __cs149_vec_int exp;
    _cs149_vload_float(base, values + i, maskAlign);
    _cs149_vload_int(exp, exponents + i, maskAlign);
    
    int cntExpNoneZero = 114514;
    while(cntExpNoneZero) {
      __cs149_mask maskExpPositive = _cs149_init_ones(0);
      _cs149_vgt_int(maskExpPositive, exp, zeros, maskAlign);
      _cs149_vmult_float(result, result, base, maskExpPositive);
      _cs149_vsub_int(exp, exp, ones, maskExpPositive);
      
      __cs149_mask maskOverflow = _cs149_init_ones(0);
      _cs149_vgt_float(maskOverflow, result, upperb, maskExpPositive);
      _cs149_vset_float(result, 9.999999f, maskOverflow);

      _cs149_vgt_int(maskExpPositive, exp, zeros, maskAlign);
      cntExpNoneZero = _cs149_cntbits(maskExpPositive);
    }
    _cs149_vstore_float(output + i, result, maskAlign);
  } 
...
float arraySumVector(float* values, int N) {
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    __cs149_vec_float currvec;
    _cs149_vload_float(currvec, values + i, maskAll);
    _cs149_vadd_float(result, currvec, result, maskAll);
  }
